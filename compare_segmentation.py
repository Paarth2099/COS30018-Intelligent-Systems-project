...
def main():
    ...
    for name in ['segment_digits.py','compare_segmentation.py','preprocess_digits.py']:
        (source_dir/name).write_bytes((Path(__file__).parent/name).read_bytes())
    rows=[]; manifest=[]
    ...
    for method in METHODS:
        ...
        summary[method]={'images':len(group),'count_matches':sum(r['count_correct'] for r in group),
            ...
            'mean_segmentation_ms':sum(r['segmentation_ms'] for r in group if r['segmentation_ms']!='')/max(1,sum(r['segmentation_ms']!='' for r in group))}
