...
def segment_image(image, method="components", max_side=1000,
    ...
    for left,top,right,bottom in boxes:
        ...
        # A component's rectangular box may overlap a neighbouring digit.
        # Remove other labelled ink, while preserving this digit's intensities.
        x0,y0,x1,y1=box
        region_labels=labels[y0:y1,x0:x1]
        if method == 'components':
            own_id=next(r['id'] for r in kept if tuple(r['box'])==(left,top,right,bottom))
            unwanted=(region_labels>0) & (region_labels!=own_id)
        else:
            unwanted=(region_labels>0) & ~clean[y0:y1,x0:x1]
        crop_pixels=np.asarray(gray.crop(box)).copy()
        background=crop_pixels[region_labels==0]
        paper=int(np.median(background)) if background.size else 255
        crop_pixels[unwanted]=paper
        crops.append(Image.fromarray(crop_pixels))
...
def recognise_number(image, predictor, method="components", representation="otsu"):
